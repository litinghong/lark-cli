...
首次使用需完成“配置应用 + 发起登录”。在飞书工作台/Agent 场景下，优先使用 `python_sdk/examples/auth.py` 的两步流程。

当你帮用户初始化配置时，使用background方式执行下面命令，启动后读取输出，从中提取授权链接并发给用户。
...
```bash
# 第一步：配置应用并发起登录（返回 device_code，不长时间阻塞）
...
当你作为 AI agent 需要帮用户完成认证时，使用两步流程，先发起授权并拿到 `device_code`，再续上轮询：

```bash
# 第一步：配置应用 + 发起授权，返回 verification_url/device_code
python3 python_sdk/examples/auth.py setup-login --new

# 第二步：用户在浏览器完成授权后，使用第一步返回的 device_code 续上轮询
python3 python_sdk/examples/auth.py login --device-code <DEVICE_CODE> --json --no-credential-file
```

规则：
- 第二步由 agent 执行，不要要求用户自行执行命令。
- 第二步建议使用长超时（>= 600s），避免短超时导致 device_code 失效。
